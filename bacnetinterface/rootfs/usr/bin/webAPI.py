...
async def operator_time_sync(
    ...
    """Broadcast datetime to all devices or to a specific device"""

    if not isinstance(date_time, datetime) and date_time is not None:
        return JSONResponse(content={"result": "Datetime not correct"}, status_code=400)

    if date_time is None:
        date_time = datetime.now()

    try:
        bacnet_application.time_sync(
            address=bacnet_application.dev_to_addr(deviceid), date_time=date_time
        )
    except Exception as err:
...
async def operator_utc_time_sync(
    ...
    """Broadcast UTC datetime to all devices or to a specific device"""
    ...
    try:
        bacnet_application.utc_time_sync(
            address=bacnet_application.dev_to_addr(deviceid), date_time=date_time
        )
    except Exception as err:
        return JSONResponse(content=jsonable_encoder({"result": str(err)}))
